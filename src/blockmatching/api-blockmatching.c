                                        ...
                                        char *param_str_1, char *param_str_2 )
{
  ...
  if ( _debug_ ) {
    ...
    BAL_PrintImage( stderr, referenceImage, "reference image" );
    BAL_PrintImage( stderr, floatingImage, "floating image" );
                                            ...
                                            char *param_str_1, char *param_str_2 )
{
    ...
    if ( par->reference_voxel.x > 0.0 && par->reference_voxel.y > 0.0 ) {
        if ( par->reference_voxel.x > 0.0 ) theReferenceImage.vx = par->reference_voxel.x;
        if ( par->reference_voxel.y > 0.0 ) theReferenceImage.vy = par->reference_voxel.y;
        if ( par->reference_voxel.z > 0.0 ) theReferenceImage.vz = par->reference_voxel.z;
        if ( BAL_SetImageVoxelSizes( &theReferenceImage, theReferenceImage.vx, theReferenceImage.vy, theReferenceImage.vz ) != 1 ) {
          if ( _verbose_ )
            fprintf( stderr, "%s: unable to change reference image voxel sizes\n", proc );
          return(-1);
        }
    }
    ...
    if ( (par->use_default_filename == 1)
      ...
      */
      if ( BAL_AllocImageFromImage( &theResultImage, (char*)NULL,
                                       ...
                                       int use_default_filename )
{
    ...
    if ( auxptr != (char*)NULL ) {
      if ( BAL_ChangeTransformationToRealUnit( floatingImage, referenceImage,
                                  ...
                                  lineCmdParamBlockmatching *p, char *str )
{
  ...
  fprintf( f, "# voxel sizes\n" );

  fprintf( f, "- reference image voxel sizes are [%f %f %f]\n",
           p->reference_voxel.x, p->reference_voxel.y, p->reference_voxel.z );
  ...
  for ( i=firstargc; i<argc; i++ ) {
    ...
    */
     ...
     */
      ...
      i ++;
      if ( i >= argc)    API_ErrorParse_blockmatching( (char*)NULL, "parsing -reference-voxel %lf", 0 );
      status = sscanf( argv[i], "%lf", &(p->reference_voxel.x) );
      if ( status <= 0 ) API_ErrorParse_blockmatching( (char*)NULL, "parsing -reference-voxel %lf", 0 );
      i ++;
      if ( i >= argc)    API_ErrorParse_blockmatching( (char*)NULL, "parsing -reference-voxel %lf %lf", 0 );
      status = sscanf( argv[i], "%lf", &(p->reference_voxel.y) );
      if ( status <= 0 ) API_ErrorParse_blockmatching( (char*)NULL, "parsing -reference-voxel %lf %lf", 0 );
      i ++;
      ...
      i ++;
      if ( i >= argc)    API_ErrorParse_blockmatching( (char*)NULL, "parsing -floating-voxel %lf", 0 );
      status = sscanf( argv[i], "%lf", &(p->floating_voxel.x) );
      if ( status <= 0 ) API_ErrorParse_blockmatching( (char*)NULL, "parsing -floating-voxel %lf", 0 );
      i ++;
      if ( i >= argc)    API_ErrorParse_blockmatching( (char*)NULL, "parsing -floating-voxel %lf %lf", 0 );
      status = sscanf( argv[i], "%lf", &(p->floating_voxel.y) );
      if ( status <= 0 ) API_ErrorParse_blockmatching( (char*)NULL, "parsing -floating-voxel %lf %lf", 0 );
      i ++;
