                                                ...
                                                bal_blockmatching_pyramidal_param *theParam )
{
  ...
  bal_image *Inrimage_flo = (bal_image *)NULL;
  bal_image smoothed_flo;
  bal_image *Inrimage_ref = (bal_image *)NULL;
  bal_image subsampled_ref;


  int allocNextTransformation = 0;
  bal_transformation *nextTransformation = (bal_transformation*)NULL;
  int allocCurrTransformation = 0;
  bal_transformation *currTransformation = (bal_transformation*)NULL;
  ...
  if ( param.verbosef != NULL ) {
    ...
    fprintf( param.verbosef, "============================================================\n" );
    fprintf( param.verbosef, "Entering %s\n", proc );
    fprintf( param.verbosef, "-------------------------------------------------------\n" );
    fprintf( param.verbosef, "---------- adjusted registration parameters -----------\n" );
    BAL_PrintBlockMatchingPyramidalParameters( param.verbosef, &param );
    fprintf( param.verbosef, "-------------------------------------------------------\n" );
    BAL_PrintImage( param.verbosef, theInrimage_ref, "Reference image" );
    BAL_PrintImage( param.verbosef, theInrimage_flo, "Floating image" );
    fprintf( param.verbosef, "\n" );
    BAL_PrintTransformation( param.verbosef, theLeftTransformation, "Left transformation" );
    BAL_PrintTransformation( param.verbosef, theInitResult, "Initial transformation" );
    fprintf( param.verbosef, "\n" );
  }
   ...
   **************************************************/
  pyramid_level = (bal_pyramid_level *)vtmalloc( (param.pyramid_highest_level+1) * sizeof(bal_pyramid_level),
                                                 "pyramid_level", proc );
  ...
  if ( param.pyramid_gaussian_filtering ) {
    if ( BAL_AllocImageFromImage( &smoothed_flo, "auxiliary_floating_image.nii",
                                      theInrimage_flo, theInrimage_flo->type ) != 1 ) {
      if ( _verbose_ )
   ...
   * 3. theLeftTransformation != NULL && theInitResult == NULL
   *    allocate theResultTransformation to identity
   *    change it to the right type
   * 4. theLeftTransformation != NULL && theInitResult != NULL
   *    set theInitResult to the right type
   *
   *
   **************************************************/
  ...
  if ( theInitResult == (bal_transformation*)NULL ) {
     ...
     */
    currTransformation = (bal_transformation*)vtmalloc( sizeof(bal_transformation),
                                                             "currTransformation", proc );
    if ( currTransformation == (bal_transformation*)NULL ) {
      if ( _verbose_ )
          fprintf( stderr, "%s: can not allocate current transformation container\n", proc );
      BAL_FreeImage( &smoothed_flo );
      vtfree( pyramid_level );
      return( (bal_transformation*)NULL );
    }

    BAL_InitTransformation( currTransformation );

    if ( BAL_AllocTransformation( currTransformation,
                                  TRANSLATION_3D,
                                  (bal_image*)NULL ) != 1 ) {
      if ( _verbose_ )
          fprintf( stderr, "%s: can not allocate current transformation\n", proc );
      vtfree( currTransformation );
      BAL_FreeImage( &smoothed_flo );
      vtfree( pyramid_level );
      return( (bal_transformation*)NULL );
    }
    allocCurrTransformation = 1;
    ...
    allocCurrTransformation = 0;
    currTransformation = theInitResult;
  }


  /* Linear transformation case
   *
   * here, we have set currTransformation
   * either to a new transformation or to a passed initial transformation
   *
   * we will not have to recompute transformations at every level
   * so we set nextTransformation = currTransformation;
   */

  if ( BAL_IsTransformationTypeLinear( param.transformation_type ) == 1 ) {

    if ( BAL_IsTransformationLinear( currTransformation ) != 1 ) {
        if ( _verbose_ )
            fprintf( stderr, "%s: requested transformation is linear, but not the initial one\n", proc );
        if ( allocCurrTransformation ) {
          BAL_FreeTransformation( currTransformation );
          vtfree( currTransformation );
        }
        BAL_FreeImage( &smoothed_flo );
    ...
    if ( currTransformation->type != param.transformation_type ) {
      /* this case may occur when the initial result transformation
       * has been read from a file
       */
      if ( BAL_ChangeTransformationType( currTransformation,
    ...
    nextTransformation = currTransformation;
  }
  ...
  for ( l=pyramid_highest_level; l>=param.pyramid_lowest_level; l-- ) {
    ...
    /* allocation and computation of the subsampled reference image
     *
     * recall that the voxel to real transformation is embedded
     * in the image structure, and that it encodes the subsampling
     * transformation
     */
    if ( l > 0 ) {
      if ( BAL_AllocComputeSubsampledImage( &subsampled_ref,
                                            pyramid_level[l].ncols,
                                            pyramid_level[l].nrows,
                                            pyramid_level[l].nplanes,
                                            theInrimage_ref,
                                            &(pyramid_level[l]), 0 ) != 1 ) {
        if ( _verbose_ )
    ...
    /* adjust transformation
     * - we have to compute new estimate from previous estimate
     * - we should have
     *     currTransformation != NULL && nextTransformation == NULL
     *
     * pay attention
     * - no desallocation of currTransformation
     *   if currTransformation = theInitResult
     *   (in this case allocCurrTransformation = 0)
     * - no allocation of nextTransformation
     *   if theInitResult != (bal_transformation*)NULL
     *
     */
    if ( nextTransformation == (bal_transformation*)NULL ) {

      /* allocation
       */

      if ( l > 0 && theInitResult != (bal_transformation*)NULL ) {
        allocNextTransformation = 0;
        nextTransformation = theInitResult;
      }
      else {

        nextTransformation = (bal_transformation*)vtmalloc( sizeof(bal_transformation),
                                                              "nextTransformation", proc );
        if ( nextTransformation == (bal_transformation*)NULL ) {
          if ( _verbose_ )
              fprintf( stderr, "%s: can not allocate next transformation container\n", proc );
          if ( l > 0 ) BAL_FreeImage( &subsampled_ref );
          if ( allocCurrTransformation ) {
            BAL_FreeTransformation( currTransformation );
            vtfree( currTransformation );
        ...
        BAL_InitTransformation( nextTransformation );

        if ( BAL_AllocTransformation( nextTransformation,
                                      param.transformation_type,
                                      Inrimage_ref ) != 1 ) {
          if ( _verbose_ )
              fprintf( stderr, "%s: can not allocate previous transformation\n", proc );
          if ( l > 0 ) BAL_FreeImage( &subsampled_ref );
          vtfree( nextTransformation );
          if ( allocCurrTransformation ) {
            BAL_FreeTransformation( currTransformation );
            vtfree( currTransformation );
          }
      ...
      if ( BAL_CopyTransformation( currTransformation, nextTransformation ) != 1 ) {
        if ( _verbose_ )
            fprintf( stderr, "%s: can not copy previous transformation into next one\n", proc );
        if ( l > 0 ) BAL_FreeImage( &subsampled_ref );
        if ( allocNextTransformation ) {
          BAL_FreeTransformation( nextTransformation );
          vtfree( nextTransformation );
        }
        if ( allocCurrTransformation ) {
          BAL_FreeTransformation( currTransformation );
      ...
      currTransformation = nextTransformation;
      allocCurrTransformation = allocNextTransformation;

      nextTransformation = (bal_transformation*)NULL;
      allocNextTransformation = 0;
    }
    ...
    if ( _debug_ >= 2 ) {
      ...
      BAL_PrintTransformation( stderr, currTransformation, "    transformation avant recalage" );
      fprintf( stderr, " ======================\n" );
      fprintf( stderr, "\n" );
    }

    if ( BAL_BlockMatching( Inrimage_ref, Inrimage_flo,
                            theLeftTransformation, currTransformation,
    ...
    if ( _debug_ >= 2 ) {
      ...
      BAL_PrintTransformation( stderr, currTransformation, "    transformation apres recalage" );
      fprintf( stderr, " ======================\n" );
      fprintf( stderr, "\n" );
    }
  ...
  /* can we returned the last computed transformation ?
   */
  if ( BAL_IsTransformationTypeLinear( param.transformation_type ) == 1 )
    return( currTransformation );

  if ( param.pyramid_lowest_level == 0 )
    return( currTransformation );


  /* there is an initial transformation
   */

  if ( theInitResult != (bal_transformation*)NULL ) {
    if ( BAL_CopyTransformation( currTransformation, theInitResult ) != 1 ) {
        if ( _verbose_ )
            fprintf( stderr, "%s: can not copy current transformation into result one\n", proc );
        if ( allocCurrTransformation ) {
          BAL_FreeTransformation( currTransformation );
          vtfree( currTransformation );
        }
  ...
  /* there is no initial transformation
   */
  ...
  if ( nextTransformation == (bal_transformation*)NULL ) {
    if ( _verbose_ )
        fprintf( stderr, "%s: can not allocate result transformation container\n", proc );
    if ( allocCurrTransformation ) {
      BAL_FreeTransformation( currTransformation );
      vtfree( currTransformation );
    }
  ...
  if ( param.verbosef != NULL ) {
    BAL_PrintTransformation( param.verbosef, nextTransformation, "Final transformation" );
    fprintf( param.verbosef, "Exiting %s\n", proc );
    fprintf( param.verbosef, "============================================================\n" );
    fprintf( param.verbosef, "\n" );
  }
                       ...
                       bal_blockmatching_param *param )
{
  ...
  if ( param->verbosef != NULL ) {
    ...
    BAL_PrintImage( param->verbosef, theInrimage_flo, "Floating image" );
    fprintf( param->verbosef, "\n" );
    BAL_PrintTransformation( param->verbosef, theLeft, "Left transformation" );
    BAL_PrintTransformation( param->verbosef, theTr, "Initial transformation" );
    fprintf( param->verbosef, "\n" );
  }
  ...
  */
  if ( BAL_AllocImageFromImage( &Inrimage_flo_sub, "subsampled_floating_image.nii",
                                    theInrimage_ref, theInrimage_flo->type ) != 1 ) {
    if ( _verbose_ ) 
  ...
  */
  if ( theTr == (bal_transformation*)NULL ) {
    ...
    if ( _verbose_ ) {
      fprintf( stderr, "%s: NULL transformation\n", proc );
      }
    return( -1 );
  }
  ...
  case VECTORFIELD_2D :
  case VECTORFIELD_3D :
    if ( param->block_dim.x % 2 != 1 ||  param->block_dim.y % 2 != 1  || param->block_dim.z % 2 != 1 ) {
      BAL_FreeImage( &Inrimage_flo_sub );
      if ( _verbose_ ) {
  ...
  */
  if ( param->write_def == 1 || param->vischeck == 1 ) {
    sprintf( image_name, "blocs_actifs_ref_py_%d.nii", param->pyramid_level );
  ...
  ************************************************************/
  for ( n_iteration = 0, rms_stop = 0;
    ...
    if ( _BALADIN_WRITE_FLOATING_IMAGE_EVOLUTION_ ) {
      sprintf( image_name, "floating_image_level%02d_iteration%02d.nii", param->pyramid_level, n_iteration );
   ...
   */ 
    ...
    */
    if ( param->write_def == 1 || param->vischeck == 1 ) {
      sprintf( image_name, "floating_blocks_level%02d_iteration%02d.nii", param->pyramid_level, n_iteration );
  ...
  if ( param->verbosef != NULL ) {
    fprintf( param->verbosef, "\n" );
    BAL_PrintTransformation( param->verbosef, theTr, "Final transformation" );
    fprintf( param->verbosef, "\n" );
    fprintf( param->verbosef, "Exiting %s\n", proc );
    fprintf( param->verbosef, "============================================================\n" );
    fprintf( param->verbosef, "\n" );
                                         ...
                                         bal_blockmatching_param *param )
{
  ...
  */
  ...
  /* borders for statistics computation
   */
  blocs_ref->border.x = param->block_border.x;
  blocs_ref->border.y = param->block_border.y;
  blocs_ref->border.z = param->block_border.z;
  ...
  */
  ...
  /* geometry
   */
  if ( BAL_CopyImageGeometryToField( image_ref, field ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to set field geometry\n", proc );
      BAL_FreeField( field );
      BAL_FreeBlocks( blocs_ref );
      BAL_FreeBlocks( blocs_flo );
      return( -1 );
...
static void  WriteImageVoxelsActifs( size_t n_allocated_blocks, BLOCS *blocs, 
                                     bal_image *Inrimage_sub, char *nom_image )
{
  char *proc = "WriteImageVoxelsActifs";
  ...
  if ( BAL_AllocImageFromImage( &Inrimage_tmp, (char*)NULL,
                           Inrimage_sub,
                           Inrimage_sub->type) != 1 ) {
    if ( _verbose_ )
